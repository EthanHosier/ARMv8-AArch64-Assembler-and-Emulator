...
static Shift *makeShiftStruct(InstructionToken shiftType, ImmediateToken magnitude) {
...
Parser_Tree *second_pass(ArrayList *list, BinarySearchTree tree) {//why return pointer?
    ...
    for (int i = 0; i < list->size; i++) {
        ...
        if (first_token->type == TOKEN_TYPE_LABEL) {//label
            ...
            returnTree->b_bCond_dotInt.immediate = ((Token) get_ArrayList_element(line, 0))->immediateToken.value;
            return returnTree;
            ...
            if (second_token->type == TOKEN_TYPE_IMMEDIATE) {//b, b.cond
                ...
                if (line->size == 2) {//br
                    ...
                    returnTree->br.Rn = *makeRegStruct(second_token->registerToken.register_name);
                } else {
                    Token third_token = get_ArrayList_element(line, 2);
                    if (third_token->type == TOKEN_TYPE_IMMEDIATE) {//cmp, cmn, neg, negs, movk, movn, movz, ldr
                        ...
                        returnTree->cmp_cmn_neg_negs_movk_movn_movz_ldrlit_IMM.R1 = *makeRegStruct(((Token) get_ArrayList_element(line, 0))->registerToken.register_name);
                        if (line->size == 5) {
                            returnTree->cmp_cmn_neg_negs_movk_movn_movz_ldrlit_IMM.shift = makeShiftStruct(((Token) get_ArrayList_element(line, 3))->instructionToken, ((Token) get_ArrayList_element(line, 4))->immediateToken);
                        }
                        return returnTree;

                    } else if (third_token->type == TOKEN_TYPE_REGISTER) {
                        if (line->size == 3 || line->size == 5 && ((Token) get_ArrayList_element(line, 3))->type == TOKEN_TYPE_INSTRUCTION) {//cmp,cmn,neg, negs, tst, mov, mvn
                            ...
                            returnTree->cmp_cmn_neg_negs_tst_mov_mvn_REG.R1 = *makeRegStruct(second_token->registerToken.register_name);
                            returnTree->cmp_cmn_neg_negs_tst_mov_mvn_REG.Rm = *makeRegStruct(third_token->registerToken.register_name);
                            if (line->size == 5) {
                                returnTree->cmp_cmn_neg_negs_tst_mov_mvn_REG.shift = makeShiftStruct(((Token) get_ArrayList_element(line, 3))->instructionToken, ((Token) get_ArrayList_element(line, 4))->immediateToken);
                            }
                            return returnTree;
                        } else {
                            Token fourth_token = get_ArrayList_element(line, 3);
                            if ((line->size == 4 || line->size == 6 && ((Token) get_ArrayList_element(line, 4))->type == TOKEN_TYPE_INSTRUCTION)) {//logic, mul, mneg
                                ...
                                if (line->size == 6) {
                                    returnTree->cmp_cmn_neg_negs_movk_movn_movz_ldrlit_IMM.shift = makeShiftStruct(
                                            ((Token) get_ArrayList_element(line, 4))->instructionToken,
                                            ((Token) get_ArrayList_element(line, 5))->immediateToken);
                                }
                                return returnTree;
                            } else if (line->size == 5 && ((Token) get_ArrayList_element(line, 4))->type == TOKEN_TYPE_REGISTER) {//madd, msub
                                returnTree->type = Type_madd_msub;
                                returnTree->madd_msub.instruction = first_token->instructionToken.instruction;
                                returnTree->madd_msub.Rd = *makeRegStruct(second_token->registerToken.register_name);
                                returnTree->madd_msub.Rn = *makeRegStruct(third_token->registerToken.register_name);
                                returnTree->madd_msub.Rm = *makeRegStruct(fourth_token->registerToken.register_name);
                                returnTree->madd_msub.Ra = *makeRegStruct(((Token) get_ArrayList_element(line, 4))->registerToken.register_name);
                            } else {
                                //error
                            }
                        }
