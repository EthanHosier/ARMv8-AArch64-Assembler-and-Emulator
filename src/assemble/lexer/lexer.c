...
static void print_arrayList_element(void *element) {
    ...
    if (strcmp(type, "ADDRESS_CODE") == 0){
        printf("ADDRESS_CODE@[%s: %s",strs[(t->addressToken.t1->type)], t->addressToken.t1->registerToken.register_name);
        if (t->addressToken.pT2 != NULL){
            char *pt2Type = strs[t->addressToken.pT2->type];
            if (strcmp(pt2Type, "REGISTER") == 0){
                printf(", %s: %s", strs[(t->addressToken.pT2->type)], t->addressToken.pT2->registerToken.register_name);
            } else if(strcmp(pt2Type, "IMMEDIATE") == 0) {
                printf(", %s: %d", strs[(t->addressToken.pT2->type)], t->addressToken.pT2->immediateToken.value);
            }
        }
        printf("]");
...
ArrayList *tokenize(char *line) {
    ...
    while (tokenStr != NULL) {
        ...
        bool exclamation = 0;
        AddressCodePossibilities result = check_if_address_code(tokenStrCopy, &exclamation);
        Token t;
